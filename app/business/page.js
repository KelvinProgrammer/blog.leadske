...
export default function BusinessPage() {
  ...
  const businessNews = articles.business || []

  // Calculate pagination
  const totalPages = Math.ceil(businessNews.length / articlesPerPage)
  const startIndex = (currentPage - 1) * articlesPerPage
  const endIndex = startIndex + articlesPerPage
  const currentArticles = businessNews.slice(startIndex, endIndex)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          <h1 className="text-4xl font-serif font-light text-foreground mb-4">Business</h1>
          ...
          {currentArticles.map((article, index) => (
            <Card key={index} className="overflow-hidden border-0 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="p-6">
                ...
                <Link href={`/business/${article.slug}`}>
