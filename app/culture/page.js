...
export default function CulturePage() {
  ...
  const cultureNews = articles.culture || []

  // Calculate pagination
  const totalPages = Math.ceil(cultureNews.length / articlesPerPage)
  const startIndex = (currentPage - 1) * articlesPerPage
  const endIndex = startIndex + articlesPerPage
  const currentArticles = cultureNews.slice(startIndex, endIndex)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          <h1 className="text-4xl font-serif font-light text-foreground mb-4">Culture</h1>
